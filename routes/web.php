...
Route::prefix('/item')->controller(ItemController::class)->middleware('auth')
            ->group(function(){
                Route::post('/', 'store')->name('item.store');
                Route::post('/update','update')->name('item.update');
                Route::post('/destroy', 'destroy')->name('item.destroy');
            });